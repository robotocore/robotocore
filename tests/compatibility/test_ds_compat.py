...
class TestDsShareDirectoryOps:
    """Tests for directory sharing operations."""

    def test_share_directory_nonexistent(self, ds):
        """ShareDirectory with fake directory raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.share_directory(
                DirectoryId="d-0000000000",
    ...
    def test_unshare_directory_nonexistent(self, ds):
        """UnshareDirectory with fake directory raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
...
class TestDsRegionOps:
    """Tests for directory region operations."""

    def test_add_region_nonexistent(self, ds):
        """AddRegion with fake directory raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.add_region(
                DirectoryId="d-0000000000",
                RegionName="us-west-2",
                VPCSettings={"VpcId": "vpc-12345", "SubnetIds": ["subnet-1", "subnet-2"]},
            )
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"

    def test_remove_region_nonexistent(self, ds):
        """RemoveRegion with fake directory raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
...
class TestDsAssessmentOps:
    """Tests for AD assessment operations."""

    def test_list_ad_assessments_empty(self, ds):
        """ListADAssessments returns Assessments list."""
        resp = ds.list_ad_assessments()
        assert "Assessments" in resp
        assert isinstance(resp["Assessments"], list)

    def test_list_ad_assessments_with_directory_id(self, ds):
        """ListADAssessments filtered by DirectoryId returns Assessments list."""
        resp = ds.list_ad_assessments(DirectoryId="d-0000000000")
        assert "Assessments" in resp
        assert isinstance(resp["Assessments"], list)
...
class TestDsDirectoryDataAccessOps:
    """Tests for directory data access operations."""

    def test_describe_directory_data_access_nonexistent(self, ds):
        """DescribeDirectoryDataAccess with fake dir raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.describe_directory_data_access(DirectoryId="d-0000000000")
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"

    def test_enable_directory_data_access_nonexistent(self, ds):
        """EnableDirectoryDataAccess with fake dir raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.enable_directory_data_access(DirectoryId="d-0000000000")
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"

    def test_disable_directory_data_access_nonexistent(self, ds):
        """DisableDirectoryDataAccess with fake dir raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.disable_directory_data_access(DirectoryId="d-0000000000")
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"


class TestDsCAEnrollmentPolicyOps:
    """Tests for CA enrollment policy operations."""

    def test_enable_ca_enrollment_policy_nonexistent(self, ds):
        """EnableCAEnrollmentPolicy with fake dir raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.enable_ca_enrollment_policy(
                DirectoryId="d-0000000000",
                PcaConnectorArn="arn:aws:pca-connector-ad:us-east-1:123456789012:connector/12345",
            )
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"

    def test_disable_ca_enrollment_policy_nonexistent(self, ds):
        """DisableCAEnrollmentPolicy with fake dir raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.disable_ca_enrollment_policy(DirectoryId="d-0000000000")
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"


class TestDsUpdateDirectorySetupOps:
    """Tests for UpdateDirectorySetup operation."""

    def test_update_directory_setup_nonexistent(self, ds):
        """UpdateDirectorySetup with fake dir raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc:
            ds.update_directory_setup(
                DirectoryId="d-0000000000",
                UpdateType="OS",
            )
        assert exc.value.response["Error"]["Code"] == "EntityDoesNotExistException"

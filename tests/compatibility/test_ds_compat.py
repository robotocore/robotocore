...
class TestDSDirectoryDataAccess:
    """Tests for Directory Data Access operations."""

    def test_enable_directory_data_access(self, ds, directory):
        """EnableDirectoryDataAccess on a directory."""
        resp = ds.enable_directory_data_access(DirectoryId=directory)
        assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200

    def test_describe_directory_data_access(self, ds, directory):
        """DescribeDirectoryDataAccess returns status."""
        resp = ds.describe_directory_data_access(DirectoryId=directory)
        assert "DataAccessStatus" in resp

    def test_disable_directory_data_access(self, ds, directory):
        """DisableDirectoryDataAccess on a directory."""
        ds.enable_directory_data_access(DirectoryId=directory)
        resp = ds.disable_directory_data_access(DirectoryId=directory)
        assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200


class TestDSCAEnrollmentPolicy:
    """Tests for CA Enrollment Policy operations."""

    _FAKE_PCA_ARN = "arn:aws:pca-connector-ad:us-east-1:123456789012:connector/fake-conn"

    def test_enable_ca_enrollment_policy(self, ds, directory):
        """EnableCAEnrollmentPolicy on a directory."""
        resp = ds.enable_ca_enrollment_policy(
            DirectoryId=directory,
            PcaConnectorArn=self._FAKE_PCA_ARN,
        )
        assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200

    def test_describe_ca_enrollment_policy(self, ds, directory):
        """DescribeCAEnrollmentPolicy returns policy info."""
        resp = ds.describe_ca_enrollment_policy(DirectoryId=directory)
        assert "RequestId" in resp.get("ResponseMetadata", {})

    def test_disable_ca_enrollment_policy(self, ds, directory):
        """DisableCAEnrollmentPolicy on a directory."""
        ds.enable_ca_enrollment_policy(
            DirectoryId=directory,
            PcaConnectorArn=self._FAKE_PCA_ARN,
        )
        resp = ds.disable_ca_enrollment_policy(DirectoryId=directory)
        assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200


class TestDSADAssessment:
    """Tests for AD Assessment operations."""

    def test_list_ad_assessments(self, ds):
        """ListADAssessments returns a list."""
        resp = ds.list_ad_assessments()
        assert "Assessments" in resp
        assert isinstance(resp["Assessments"], list)

    def test_start_ad_assessment(self, ds, directory):
        """StartADAssessment starts an assessment."""
        resp = ds.start_ad_assessment(DirectoryId=directory)
        assert "AssessmentId" in resp

    def test_describe_ad_assessment(self, ds, directory):
        """DescribeADAssessment for a started assessment."""
        start_resp = ds.start_ad_assessment(DirectoryId=directory)
        assessment_id = start_resp["AssessmentId"]
        resp = ds.describe_ad_assessment(AssessmentId=assessment_id)
        assert "Assessment" in resp
        assert resp["Assessment"]["AssessmentId"] == assessment_id

    def test_delete_ad_assessment(self, ds, directory):
        """DeleteADAssessment removes an assessment."""
        start_resp = ds.start_ad_assessment(DirectoryId=directory)
        assessment_id = start_resp["AssessmentId"]
        resp = ds.delete_ad_assessment(AssessmentId=assessment_id)
        assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200
...
class TestDSSchemaExtension:
    """Tests for schema extension operations."""

    def test_start_schema_extension(self, ds, directory):
        """StartSchemaExtension starts an extension."""
        resp = ds.start_schema_extension(
            DirectoryId=directory,
            CreateSnapshotBeforeSchemaExtension=False,
            LdifContent="dn: CN=test,DC=corp,DC=example,DC=com\nchangetype: add\n",
            Description="Test schema extension",
        )
        assert "SchemaExtensionId" in resp

    def test_cancel_schema_extension(self, ds, directory):
        """CancelSchemaExtension cancels a running extension."""
        start_resp = ds.start_schema_extension(
            DirectoryId=directory,
            CreateSnapshotBeforeSchemaExtension=False,
            LdifContent="dn: CN=test2,DC=corp,DC=example,DC=com\nchangetype: add\n",
            Description="Cancel test",
        )
        ext_id = start_resp["SchemaExtensionId"]
        resp = ds.cancel_schema_extension(
            DirectoryId=directory,
            SchemaExtensionId=ext_id,
        )
        assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200


class TestDSRegionOps:
    """Tests for multi-region directory operations."""

    def test_add_region_requires_microsoft_ad(self, ds, directory, ec2):
        """AddRegion on SimpleAD raises UnsupportedOperationException (proves server contact)."""
    ...
    def test_remove_region_nonexistent(self, ds):
        """RemoveRegion with nonexistent directory raises EntityDoesNotExistException."""
        with pytest.raises(ClientError) as exc_info:
            ds.remove_region(DirectoryId="d-0000000000")
        assert exc_info.value.response["Error"]["Code"] == "EntityDoesNotExistException"
...
class TestDSUpdateOps:
    ...
    def test_update_number_of_domain_controllers_requires_msad(self, ds, directory):
        """UpdateNumberOfDomainControllers on SimpleAD raises UnsupportedOperationException."""
        with pytest.raises(ClientError) as exc_info:
            ds.update_number_of_domain_controllers(
                DirectoryId=directory,
                DesiredNumber=2,
            )
        assert exc_info.value.response["Error"]["Code"] == "UnsupportedOperationException"

    def test_update_directory_setup(self, ds, directory):
        """UpdateDirectorySetup updates directory setup."""

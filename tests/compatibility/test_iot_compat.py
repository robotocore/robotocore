...
class TestIoTDescribeAndListOperations:
    def test_describe_account_audit_configuration(self, iot):
        """DescribeAccountAuditConfiguration returns audit config."""
        resp = iot.describe_account_audit_configuration()
        assert (
            "auditCheckConfigurations" in resp or resp["ResponseMetadata"]["HTTPStatusCode"] == 200
        )

    def test_describe_event_configurations(self, iot):
        """DescribeEventConfigurations returns event config."""
        resp = iot.describe_event_configurations()
        assert "eventConfigurations" in resp or resp["ResponseMetadata"]["HTTPStatusCode"] == 200

    def test_describe_index(self, iot):
        """DescribeIndex returns index info."""
        try:
            resp = iot.describe_index(indexName="AWS_Things")
            assert "indexName" in resp
    ...
    def test_describe_default_authorizer(self, iot):
        """DescribeDefaultAuthorizer returns error when none set."""
        with pytest.raises(ClientError) as exc:
            iot.describe_default_authorizer()
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_audit_finding(self, iot):
        """DescribeAuditFinding with fake ID returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_audit_finding(findingId="fake-finding-id-12345678")
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_audit_task(self, iot):
        """DescribeAuditTask with fake ID returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_audit_task(taskId="fake-task-id-12345678")
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_audit_suppression(self, iot):
        """DescribeAuditSuppression with fake params returns error."""
        with pytest.raises(ClientError) as exc:
    ...
    def test_describe_audit_mitigation_actions_task(self, iot):
        """DescribeAuditMitigationActionsTask with fake ID returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_audit_mitigation_actions_task(taskId="fake-task-id-12345678")
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_detect_mitigation_actions_task(self, iot):
        """DescribeDetectMitigationActionsTask with fake ID returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_detect_mitigation_actions_task(taskId="fake-task-id-12345678")
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_managed_job_template(self, iot):
        """DescribeManagedJobTemplate with fake name returns error."""
        with pytest.raises(ClientError) as exc:
    ...
    def test_describe_scheduled_audit(self, iot):
        """DescribeScheduledAudit with fake name returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_scheduled_audit(scheduledAuditName="fake-audit")
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_stream(self, iot):
        """DescribeStream with fake ID returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_stream(streamId="fake-stream-id")
        assert exc.value.response["Error"]["Code"] == "ResourceNotFoundException"

    def test_describe_thing_registration_task(self, iot):
        """DescribeThingRegistrationTask with fake ID returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_thing_registration_task(taskId="fake-task-id-12345678")
        assert exc.value.response["Error"]["Code"] in (
            "ResourceNotFoundException",
            "InvalidRequestException",
        )
    ...
    def test_describe_provisioning_template_version(self, iot):
        """DescribeProvisioningTemplateVersion with fake template returns error."""
        with pytest.raises(ClientError) as exc:
            iot.describe_provisioning_template_version(

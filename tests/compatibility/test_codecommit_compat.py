...
class TestCodeCommitEdgeCases:
    ...
    def test_evaluate_pr_approval_rules_after_adding_rule(self, codecommit):
        """EvaluatePullRequestApprovalRules marks unapproved rules as not satisfied."""
        name, pr_id, rev_id, *_ = _create_pr(codecommit)
        try:
            codecommit.create_pull_request_approval_rule(
                pullRequestId=pr_id,
                approvalRuleName="needs-approval",
                approvalRuleContent=APPROVAL_RULE_CONTENT,
            )
            resp = codecommit.evaluate_pull_request_approval_rules(
                pullRequestId=pr_id, revisionId=rev_id
            ...
            assert "needs-approval" in evaluation["approvalRulesNotSatisfied"]
            assert evaluation["approvalRulesSatisfied"] == []
        finally:
            codecommit.delete_repository(repositoryName=name)

    def test_batch_get_commits_invalid_id_in_errors(self, codecommit):
        """BatchGetCommits returns invalid IDs in errors list."""
        name, commit_id = _create_repo_with_commit(codecommit)
        try:
            resp = codecommit.batch_get_commits(
                commitIds=[commit_id, "deadbeef" * 5],
                repositoryName=name,
            )
            assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200
            # Valid commit should be in commits list
            returned_ids = {c["commitId"] for c in resp["commits"]}
            assert commit_id in returned_ids
            # Invalid commit should be reported in errors
            assert "errors" in resp
            error_ids = [e["commitId"] for e in resp["errors"]]
            assert "deadbeef" * 5 in error_ids
        finally:
            codecommit.delete_repository(repositoryName=name)

    def test_merge_branches_by_fast_forward_updates_destination_head(self, codecommit):
        """After FF merge, destination branch HEAD matches source commit."""
        name, main_commit, feature_commit = _create_repo_with_feature_branch(codecommit)
        try:
            ...
            # main branch head should now match feature commit
            branch = codecommit.get_branch(repositoryName=name, branchName="main")
            assert branch["branch"]["commitId"] == feature_commit
        finally:
            codecommit.delete_repository(repositoryName=name)

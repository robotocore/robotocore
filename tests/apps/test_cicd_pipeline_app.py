...
class TestArtifactManagement:
    ...
    def test_artifact_metadata(self, s3, artifact_bucket):
        ...
        assert resp["Metadata"]["commit_sha"] == "abc123def456"
        assert resp["Metadata"]["branch"] == "feature/login"
        assert resp["Metadata"]["build_number"] == "42"
...
class TestBuildLogs:
    ...
    def test_stream_build_output(self, logs, build_logs):
        ...
        events = [
            {"timestamp": 1709900000000 + i * 1000, "message": f"[step {i}] Building..."}
            for i in range(10)
        ]
        logs.put_log_events(
    ...
    def test_filter_error_logs(self, logs, build_logs):
        ...
        events = [
            {"timestamp": 1709900000000 + i * 1000, "message": msg}
...
class TestPipelineOrchestration:
    ...
    def test_full_build_pipeline(
        ...
        logs.put_log_events(
            ...
            logStreamName=stream_name,
            logEvents=[
                {"timestamp": 1709900000000, "message": f"[{build_id}] Build started"},
                {"timestamp": 1709900001000, "message": f"[{build_id}] Tests passed"},
                {"timestamp": 1709900002000, "message": f"[{build_id}] Build complete"},
            ],
        )
        ...
        try:
            ...
            assert art_resp["Metadata"]["build_id"] == build_id

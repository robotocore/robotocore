...
LOCALSTACK_HOST = "localhost.localstack.cloud"  # backward-compat alias
...
# Host suffix pattern matching both robotocore.cloud and localstack.cloud (compat alias)
_HOST_SUFFIX = r"localhost\.(?:robotocore|localstack)\.cloud"

# Matches Host: sqs.{region}.localhost.robotocore.cloud (or localstack.cloud)
STANDARD_HOST_RE = re.compile(r"^sqs\.(?P<region>[a-z0-9-]+)\." + _HOST_SUFFIX)

# Matches Host: {region}.queue.localhost.robotocore.cloud (or localstack.cloud)
DOMAIN_HOST_RE = re.compile(r"^(?P<region>[a-z0-9-]+)\.queue\." + _HOST_SUFFIX)
...
def parse_sqs_url(path: str, host: str) -> dict | None:
    ...
    Accepts both robotocore.cloud and localstack.cloud hostnames.
    """

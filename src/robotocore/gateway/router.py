...
def route_to_service(request: Request) -> str | None:
    ...
    if ".s3." in host or host.startswith("s3.") or host.startswith("s3-"):
        return "s3"
    # SQS endpoint strategy host patterns (robotocore.cloud primary, localstack.cloud compat)
    if (
        ...
        return "sqs"
    # OpenSearch endpoint strategy host patterns (robotocore.cloud primary, localstack.cloud compat)
    if (
        ".opensearch.localhost.robotocore.cloud" in host
        or ".opensearch.localhost.localstack.cloud" in host
    ):
        return "opensearch"
